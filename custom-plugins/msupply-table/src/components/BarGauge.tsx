...
// Utils
import { measureText, calculateFontSize } from '../utils';
...
export interface BarGaugeProps extends Themeable {
  height: number;
  width: number;
  field: FieldConfig;
...
export class BarGauge extends PureComponent<BarGaugeProps> {
  static defaultProps: Partial<BarGaugeProps> = {
    lcdCellWidth: 12,
    value: {
      text: '100',
      numeric: 100,
...
function calculateTitleDimensions(props: BarGaugeProps): TitleDimensions {
...
export function getTitleStyles(props: BarGaugeProps): { wrapper: CSSProperties; title: CSSProperties } {
...
function calculateBarAndValueDimensions(props: BarGaugeProps): BarAndValueDimensions {
 ...
 */
export function getBasicAndGradientStyles(props: BarGaugeProps): BasicAndGradientStyles {
 ...
 * Only exported to for unit test
 */
export function getBarGradient(props: BarGaugeProps, maxSize: number): string {
 ...
 * Only exported to for unit test
 */
export function getValueColor(props: BarGaugeProps): string {
  const { value } = props;
  if (value.color) {
    return value.color;
  }
